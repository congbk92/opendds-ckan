...
def main():
    if (len(sys.argv) > 1):
        if sys.argv[1] == "check":
            ...
            print(parse(sys.argv[2:], 'c:', ["config="])[0])
            print(open(parse(sys.argv[2:], 'c:', ["config="])[0]).read())
            ...
            if config['common']['converter'] == 'dds':
                if config['dds']['mode'] == 'subscriber':
                    dtStore = DataStore()
                    dtStore.addDataToColumn("Topic", "LifetimeDogLicenses")
                    dtStore.addDataToColumn("idl", config['dds']['file_idl'])
                    dtStore.addDataToColumn("netConfig", config['dds']['network_config'])
                    #ckan = CKANConnector("http://localhost:5000", "b3c5e73a-52ef-4156-96eb-ef022746ea74", package_id="f4b61669-67d7-48c4-b114-79170f37cb5f")
                    ckan = CKANConnector(config['common']['ckan_sever'], config['common']['api_token'], config['common']['resource_id'])
